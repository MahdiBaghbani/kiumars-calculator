...
use crate::buttons::action_button::ActionButton;
use crate::buttons::button_styles::get_button_styles;
use crate::buttons::number_button::NumberButton;
...
pub fn app() -> Html {
    ...
    let clear_all = {
        !((*temp_value).is_some() || (*action).is_some() || *trailing_dot)
    };
    ...
    html! {
      <div class={classes!("h-screen", "w-screen", "bg-background", "flex-col", "flex")}>
        <div class={classes!("flex", "justify-end", "text-5xl", "pr-2", "py-10")}>
        ...
        <div class={classes!("flex", "flex-row", "flex-1")}>
            <NumberButton value={7.0}
              onclick={handle_number_button.clone()} />
            <NumberButton value={8.0}
              onclick={handle_number_button.clone()} />
            <NumberButton value={9.0}
              onclick={handle_number_button.clone()} />
            <ActionButton action={Action::Multiply}
              onclick={handle_action_button.clone()} selected_action={*action} />
        </div>
        <div class={classes!("flex", "flex-row", "flex-1")}>
